...
Un bouton **Imprimer** est disponible en haut de chaque page (à côté du badge
LIVE) : la mise en page bascule automatiquement en thème clair pleine largeur
pour l'impression, sidebar et boutons d'action masqués.
...
| **Prospects** | Suivi de prospection commerciale : tableau éditable en ligne (entreprise, statut, démo, RDV, notes), tri par colonne, recherche, en-tête figé + KPI et graphiques de répartition avec pourcentages |
...
## 6. Connexion (Firebase Authentication) et contrôle d'accès par rôle
...
pour vous connecter à R.I.S.E.

⚠️ **Point important** : RISE Presence et FleetGest (toutes ses déclinaisons
— G4S, SOCOB, MyGestStock...) partagent le **même projet Firebase**, donc le
**même annuaire de comptes**. Sans protection supplémentaire, n'importe quel
compte client FleetGest valide pourrait se connecter à RISE Presence et voir
les données de tous vos clients.

C'est pour ça qu'après la connexion, l'app vérifie le **rôle** du compte dans
`users/{uid}.role` : seuls `"Administrateur"` et `"Agent"`
(`RISE_PRESENCE_ALLOWED_ROLES` dans `src/config/presenceSchema.js`) peuvent
accéder au tableau de bord. Un compte `"Client"` — ou sans profil du tout —
voit un écran "Accès refusé" avec un bouton pour se déconnecter, quel que
soit le mot de passe utilisé.
